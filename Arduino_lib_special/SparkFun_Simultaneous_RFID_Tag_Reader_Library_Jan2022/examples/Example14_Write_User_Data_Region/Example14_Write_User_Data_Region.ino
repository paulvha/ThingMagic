...
void loop()
{
  ...
  while (Serial.available()) {Serial.read();delay(10);}  // Throw away the user's characters
  ...
  uint32_t WordStartAdress = 0x4;     // offset 4 x words = start 4 x 2 bytes skip = start at byte  9

  byte responseType = nano.writeDataRegion(TMR_GEN2_BANK_USER, WordStartAdress, testData, sizeof(testData));
