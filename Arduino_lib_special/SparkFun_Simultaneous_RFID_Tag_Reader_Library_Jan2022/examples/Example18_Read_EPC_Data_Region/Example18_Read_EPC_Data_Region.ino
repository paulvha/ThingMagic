/*
  ...
  You can now select to read a specific area of the EPC only.
...
void setup()
{
  ...
  Serial.println(F("Example18: read EPC data region"));
...
void loop()
{
  Serial.println(F("Press a key to read EPC data"));
  ...
  byte responseType;
  byte myData[64];
  byte myDataLength = sizeof(myData);  //Tell readDataRegion to read max up to 64 bytes

  uint32_t WordStartAdress = 1;       // 0x1 : that is where PC starts
  uint8_t RegionLength = 0x7;         // number of words to read 0x7 = 7 words = 14 bytes (PC + EPC)

  responseType = nano.readDataRegion(TMR_GEN2_BANK_EPC, WordStartAdress, RegionLength, myData, myDataLength);
